...
describe('uploadChangesToS3', () => {
  ...
  it('throws if changes is not an array', async () => {
    await expect(uploadChangesToS3('not-an-array', 'key.txt')).rejects.toThrow(
      'Changes must be an array before uploading to S3'
    );
  });
  ...
  it('writes file and calls uploadToS3 when changes exist', async () => {
    ...
    const result = await uploadChangesToS3(
      changes,
      'changes/tps/2025/file.txt'
    );
